...
def fit_gamma(h, eng, offset=0, offset_sigma=10):
    ...
    #_ ,peak = ROOT_peaks(h,width=20,height=0.05,options='nobackground')
    ...
    f.SetParameter(1, 0.08*peak)
    f.SetParameter(2, h.GetBinContent(h.FindBin(peak)))
    r = h.Fit(f, 'QLSB+', '', peak-0.15*peak, peak+0.30*peak)
    #h.Write()
    ...
    r = h.Fit(f, 'QLSB+', '', eng*f.GetParameter(0) - 0.8*abs(f.GetParameter(1)), eng*f.GetParameter(0) + 1.*abs(f.GetParameter(1)))
    f.Write()
    h.Write()
    try:
