...
def fit_gamma(h, eng, offset=0, offset_sigma=10):
    ...
    r = h.Fit(f, FIT_OPTIONS, '', peak-5*sigma, peak+5*sigma)
    h.Write()
    # Secondary fit that limits the range to +/-1 sigma. This makes the model
    ...
    # reasons.
    r = h.Fit(f, FIT_OPTIONS, '', eng*f.GetParameter(0) - abs(f.GetParameter(1)), eng*f.GetParameter(0) + abs(f.GetParameter(1)))
    f.Write()
    h.Write()
    try:
